...
This script stack files listed in the input list (ex: list_1ne.txt). This script is for January data.
...
Optional arguments and their default values are (case insensitive):
	...
	-POS_MERR1=5.0			Max position uncertainty for SCAMP 1st UNCHANGED (arcmin)

	-POS_MERR2=5.0			Max position uncertainty for SCAMP 2nd UNCHANGED (arcmin)
...
if [ $scamp_TF == "true" ]; then
	for file1 in `cat ${path}cats_for_scamp/${cluster_name}_${filter_location}_cats_for_scamp_list.txt`; do
		...
		scamp -MOSAIC_TYPE UNCHANGED -ASTREF_CATALOG $SCAMP_CATALOG -POSITION_MAXERR $POS_MERR1 -VERBOSE_TYPE $SCAMP_VERBOSE -HEADER_NAME ${path}${name}.head -c ${script_dir}/extra/default.scamp ${path}cats_for_scamp/$file1 2>&1 | tee ${path}scamp_logs/${name2}_scamp_log1.txt
		mv ${path}${name}.head ${path}${name}.ahead
		scamp -MOSAIC_TYPE UNCHANGED -ASTREF_CATALOG $SCAMP_CATALOG -POSITION_MAXERR $POS_MERR2 -AHEADER_NAME ${path}${name}.ahead -VERBOSE_TYPE $SCAMP_VERBOSE -HEADER_NAME ${path}${name}.head -c ${script_dir}/extra/default.scamp ${path}cats_for_scamp/$file1 2>&1 | tee ${path}scamp_logs/${name2}_scamp_log2.txt
#		mv ${path}${name}.head ${path}${name}.ahead
...
ave_gain=`gethead gain ${path}${name2}.fits`
